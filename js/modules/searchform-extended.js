...
jQuery(document).ready(function($) {
  ...
  if ($('.exhibitor-search-form').length !== 1) return;
  ...
  */
  ...
  $panel_filters.hide();
  $panel_header.on('click.esf', function() {
    $(this).toggleClass('is-closed');
    $panel_filters.slideToggle(300);
  });
  $panel_header.on('keypress.esf', function(e) {
  ...
  $('.esf-as-list-filters').find('a').on('click.esf', function() {
  ...
  // on checkbox check
  $panel_content.find(':checkbox').on('change.esf', function() {

    var $ancestors = false;

    if ($(this).is(':checked')) { // if checkbox is checked

      $(this).closest('li').addClass('esf-item-checked').addClass('esf-item-selected').find('ul li').addClass('esf-item-checked').find('input:checkbox').prop('checked', true);
      ...
      // Remove highlight of children
      $(this).closest('li').find('.esf-has-sub-items').removeClass('esf-item-selected');

      esf_count_selected($(this));
    }

    // Remove highlight of ancestors (only if not highlighted by other item)
    $ancestors = $(this).parentsUntil('.esf-tree').filter('.esf-has-sub-items');
    $ancestors.each(function() {
      var $checked_childs = $(this).find('.esf-choice-container :checkbox:checked').not('.esf-checkbox-select-all');
      if($checked_childs.length===0) {
        $(this).removeClass('esf-item-selected').find('.esf-as-count').remove();
      } else {
        var $label = $(this).addClass('esf-item-selected').find('>.esf-choice-container label');
        $label.find('.esf-as-count').remove();
        $label.append(' <span class="esf-as-count">('+($checked_childs.length)+' sélectionné'+($checked_childs.length>1?'s':'')+')</span>');
      }
    });
  ...
  $panel_content.on('change.esf', '.esf-checkbox-select-all', function() {
    if ($(this).is(':checked')) {
  ...
  // on next lvl click
  $panel_content.on('click.esf', '.esf-has-sub-items > .esf-choice-container .esf-next-lvl', function() {
  ...
  // on prev lvl click
  $panel_content.on('click.esf', '.esf-back-to-prev-lvl', function() {
    var $_this = $(this),
  ...
  $panel.on('click.esf', '.esf-remove-filters', function() {
  ...
  $('.esf-as-af-filters').on('click.esf', '.esf-tag-filter-remove', function() {
  ...
  $('.esf-results-alpha-list').find('a').on('click.esf', function() {
    // AJAX update list
    esf_AJAX_update_list();
    return false;
  });
